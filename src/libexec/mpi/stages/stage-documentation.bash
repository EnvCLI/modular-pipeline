...
function main()
{
  ...
  if [ "$NCI_COMMIT_REF_TYPE" == "tag" ]; then
    ...
    if [ -n "$RELEASE_WEBHOOK_DISCORD" ]; then
      CHANGELOG_CONFIG_FILE="$TMP_DIR/changelog/publish-release-markdown-discord.yml" @mpi action changelog-generate --ref="$NCI_COMMIT_REF_NAME" --output="${TMP_DIR}/changelog-discord"
      @mpi.discord.send_webhook "RELEASE_WEBHOOK_DISCORD" "Releasebot" "$(<${TMP_DIR}/changelog-discord)"
    fi
    # - rocketchat
